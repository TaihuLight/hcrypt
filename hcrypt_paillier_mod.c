...
static int mod_paillier_keygen(paillier_pubkey_t **pk, paillier_prvkey_t **sk)
{
...
{
	paillier_pubkey_t *pub_key = NULL;
	paillier_prvkey_t *prv_key = NULL;
	...
	if (mod_paillier_keygen(&pub_key, &prv_key) < 0) {
		OPENSSL_free(*pk);
		OPENSSL_free(*sk);
		*pk = NULL;
		*sk = NULL;
	...
	(*pk)->algor = hcrypt_paillier();
	(*sk)->algor = hcrypt_paillier();
	(*pk)->u.paillier = pub_key;
	(*sk)->u.paillier = prv_key;
	...
	hcrypt_pubkey_t *pk)
{
	...
	OPENSSL_assert(ct->algor == hcrypt_paillier());
	OPENSSL_assert(pk->algor == hcrypt_paillier());
	OPENSSL_assert(pt->algor == hcrypt_paillier());
	OPENSSL_assert(ct->u.paillier);
	OPENSSL_assert(pt->u.paillier);
	OPENSSL_assert(pk->u.paillier);

	if (paillier_encrypt(ct->u.paillier, pt->u.paillier, pk->u.paillier) < 0) {
	...
	hcrypt_prvkey_t *sk)
{
	...
	OPENSSL_assert(pt->algor == hcrypt_paillier());
	OPENSSL_assert(ct->algor == hcrypt_paillier());
	OPENSSL_assert(sk->algor == hcrypt_paillier());
	OPENSSL_assert(pt->u.paillier);
	OPENSSL_assert(ct->u.paillier);
	OPENSSL_assert(sk->u.paillier);

	if (paillier_decrypt(pt->u.paillier, ct->u.paillier, sk->u.paillier) < 0) {
	...
	const hcrypt_ciphertext_t *b, hcrypt_pubkey_t *pk)
{
	...
	OPENSSL_assert(r->algor == hcrypt_paillier());
	OPENSSL_assert(a->algor == hcrypt_paillier());
	OPENSSL_assert(b->algor == hcrypt_paillier());
	OPENSSL_assert(pk->algor == hcrypt_paillier());
	OPENSSL_assert(r->u.paillier);
	OPENSSL_assert(a->u.paillier);
	OPENSSL_assert(b->u.paillier);
	OPENSSL_assert(pk->u.paillier);
	
	if (paillier_ciphertext_add(r->u.paillier, a->u.paillier,  b->u.paillier, pk->u.paillier) < 0) {
...
static int mod_pubkey_new(hcrypt_pubkey_t **pk)
{
	...
	if (!(pub_key = paillier_pubkey_new())) {
		OPENSSL_free(*pk);
		*pk = NULL;
		return -1;
	}
...
static int mod_pubkey_set_str(hcrypt_pubkey_t *pk, const char *str)
{
	...
	return paillier_pubkey_set_str(pk->u.paillier, str);
}
...
static int mod_pubkey_to_str(const hcrypt_pubkey_t *pk, char *buf, size_t len)
{
	...
	return paillier_pubkey_to_str(pk->u.paillier, buf, len);
}
...
static int mod_pubkey_free(hcrypt_pubkey_t *pk)
{
	...
	if (pk->u.paillier) {
		paillier_pubkey_free(pk->u.paillier);
	}
...
static int mod_prvkey_new(hcrypt_prvkey_t **sk)
{
	...
	if (!(prv_key = paillier_prvkey_new())) {
		OPENSSL_free(*sk);
		*sk = NULL;
		return -1;
	}
...
static int mod_prvkey_set_str(hcrypt_prvkey_t *sk, const char *str)
{
	...
	return paillier_prvkey_set_str(sk->u.paillier, str);
}
...
static int mod_prvkey_to_str(const hcrypt_prvkey_t *sk, char *buf, size_t len)
{
	...
	return paillier_prvkey_to_str(sk->u.paillier, buf, len);
}
...
static int mod_prvkey_free(hcrypt_prvkey_t *sk)
{
	...
	if (sk->u.paillier) {
		paillier_prvkey_free(sk->u.paillier);
	}
...
static int mod_plaintext_new(hcrypt_plaintext_t **pt)
{
	...
	if (!(m = paillier_plaintext_new())) {
		OPENSSL_free(*pt);
		*pt = NULL;
		return -1;
	}

	(*pt)->algor = hcrypt_paillier();
	(*pt)->u.paillier = m;
...
static int mod_plaintext_set_str(hcrypt_plaintext_t *pt, const char *str)
{
	...
	OPENSSL_assert(pt->u.paillier);
 	...
 	a = atoi(str);
	return paillier_plaintext_set_word(pt->u.paillier, a);
}
...
static int mod_plaintext_to_str(const hcrypt_plaintext_t *pt, char *buf, size_t len)
{
	...
	if (paillier_plaintext_to_word(pt->u.paillier, &a) < 0) {
		fprintf(stderr, "%s %d\n", __FILE__, __LINE__);
		return -1;
	}
...
static int mod_plaintext_to_word(const hcrypt_plaintext_t *pt, unsigned long *a)
{
	...
	return paillier_plaintext_to_word(pt->u.paillier, a);
}
...
static int mod_plaintext_free(hcrypt_plaintext_t *pt)
{
	...
	if (pt->u.paillier) {
		paillier_plaintext_free(pt->u.paillier);
	}
...
static int mod_ciphertext_new(hcrypt_ciphertext_t **ct, hcrypt_pubkey_t *pk)
{
	...
	if (!(c = paillier_ciphertext_new())) {
		OPENSSL_free(*ct);
		*ct = NULL;
		return -1;
	}

	(*ct)->algor = hcrypt_paillier();
	(*ct)->u.paillier = c;
...
static int mod_ciphertext_set_str(hcrypt_ciphertext_t *ct, const char *str)
{
	...
	return paillier_ciphertext_set_str(ct->u.paillier, str);
}
...
static int mod_ciphertext_to_str(const hcrypt_ciphertext_t *ct, char *buf, size_t len)
{
	...
	return paillier_ciphertext_to_str(ct->u.paillier, buf, len);
}
...
static int mod_ciphertext_free(hcrypt_ciphertext_t *ct)
{
	...
	if (ct->u.paillier) {
		paillier_ciphertext_free(ct->u.paillier);
	}
...
static const hcrypt_algor_t paillier_algor = {
	"paillier",
	mod_keygen, /* keygen */
	mod_encrypt, /* encrypt */
	mod_decrypt, /* decrypt */
